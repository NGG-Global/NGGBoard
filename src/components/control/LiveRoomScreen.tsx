...
export function LiveRoomScreen({ roomId }: { roomId: string }) {
  ...
  if (!room || !board) {
    return (
      <div dir="rtl" style={{ height: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, background: "var(--surface-sunken)", fontFamily: "var(--font-sans)", textAlign: "center", padding: 24 }}>
  ...
  const facilitator: FacilitatorControls = {
    ...
    onHide: (id) => {
      const s = submissions.find((x) => x.id === id);
      if (s) moderateWithUndo(s, "hide", "התוכן הוסתר מהמסך");
    },
    onDelete: (id) => {
      const s = submissions.find((x) => x.id === id);
      if (s) moderateWithUndo(s, "delete", "התוכן נמחק");
    },
  };
  ...
  function listActionsFor(sub: Submission): CardActions {
    if (tab === "pending") {
      return {
        onApprove: () => { db.moderate(sub.id, "approve"); toast.show("התוכן אושר ומוצג על המסך"); },
    ...
    return {
      onRestore: () => { db.moderate(sub.id, "restore"); toast.show("התוכן הושב לתצוגה"); },
      onDelete: () => moderateWithUndo(sub, "delete", "התוכן נמחק"),
    };
  }
  ...
  return (
    <div dir="ltr" style={{ height: "100vh", width: "100vw", display: "flex", overflow: "hidden", background: "var(--ink-950)", fontFamily: "var(--font-sans)" }}>
      ...
      <aside
        ...
        aria-hidden={!open}
      >
        <div dir="rtl" style={{ width: DRAWER_W, height: "100%", display: "flex", flexDirection: "column", color: "var(--text)" }}>
          {/* Drawer header */}
          <div style={{ flex: "none", padding: "14px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10 }}>
            <button onClick={() => router.push(`/app/boards/${board.id}`)} className="ngg-hover" aria-label="חזרה ללוח" style={{ border: "none", background: "transparent", cursor: "pointer", padding: 6, borderRadius: "var(--radius-md)", color: "var(--text-muted)", display: "flex" }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ fontSize: "var(--text-2xs)", color: "var(--text-subtle)" }}>
                {room.session_label ? `${room.session_label} · ` : ""}החל לפני {elapsed} דק׳
              </div>
            </div>
            <RoomStatusBadge status={room.status} solid={liveNow} />
            <button onClick={() => setOpen(false)} className="ngg-hover" aria-label="סגור בקרה" title="סגור פאנל" style={{ border: "none", background: "transparent", cursor: "pointer", padding: 6, borderRadius: "var(--radius-md)", color: "var(--text-muted)", display: "flex" }}>
              <IconX size={16} />
            </button>
          </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {inactivity.warning && liveNow && (
              <Alert color="warning" icon={<IconWarning size={15} />}>
                אין פעילות {Math.round(inactivity.idleMs / 60000)} דק׳ — החדר יושהה בעוד {inactivity.minutesUntilSuspend} דק׳
                <Button variant="secondary" size="sm" onClick={() => db.touchActivity(roomId)}>שמור פעיל</Button>
              </Alert>
            )}
            {suspended && (
              <Alert color="warning" icon={<IconClock size={15} />}>
                החדר הושהה אוטומטית לאחר 30 דקות ללא פעילות. התוכן נשמר.
                <Button variant="primary" size="sm" onClick={() => { db.reactivateRoom(roomId); toast.show("החדר הופעל מחדש"); }}>הפעל מחדש</Button>
              </Alert>
            )}

            {/* Join panel */}
            <SectionCard title="הצטרפות למפגש">
              <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 0 }}>
                  ...
                  <button onClick={() => { navigator.clipboard?.writeText(joinUrl).catch(() => {}); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className="ngg-hover" style={{ display: "inline-flex", alignItems: "center", gap: 4, border: "none", background: "transparent", color: "var(--accent-text)", fontSize: "var(--text-2xs)", fontWeight: "var(--weight-bold)", cursor: "pointer", padding: "3px 4px", borderRadius: "var(--radius-md)", alignSelf: "flex-start" }}>
                    <IconCopy size={13} /> {copied ? "הועתק" : "העתק קישור"}
                  </button>
                </div>
              </div>
              <button
            ...
            {/* Room controls */}
            <SectionCard title="בקרת חדר">
              {!suspended && (
                <div style={{ display: "flex", gap: 8 }}>
                  <ToggleBtn active={paused} onClick={() => { if (paused) { db.resumeRoom(roomId); toast.show("קבלת התוכן חודשה"); } else { db.pauseRoom(roomId); toast.show("קבלת התוכן הושהתה"); } }} icon={paused ? <IconPlay size={14} /> : <IconPause size={14} />} color="warning">
                    {paused ? "המשך קבלה" : "השהה קבלה"}
                  </ToggleBtn>
                  <ToggleBtn active={readOnly} onClick={() => { db.setReadOnly(roomId, !readOnly); toast.show(readOnly ? "מצב קריאה בלבד בוטל" : "מצב קריאה בלבד"); }} icon={<IconEyeOff size={14} />} color="info">
                    קריאה בלבד
                  </ToggleBtn>
                </div>
              )}
              <div style={{ fontSize: "var(--text-2xs)", fontWeight: "var(--weight-bold)", color: "var(--text-subtle)", marginTop: 2 }}>פריסת תצוגה</div>
                ...
                {(["wall", "mosaic", "feed"] as DisplayLayout[]).map((l) => {
                  ...
                  return (
                    <button key={l} onClick={() => db.setLayout(roomId, l)} aria-pressed={active} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 5, padding: "9px 4px", borderRadius: "var(--radius-md)", border: `1.5px solid ${active ? "var(--magenta-500)" : "var(--border)"}`, background: active ? "var(--accent-soft)" : "var(--surface)", color: active ? "var(--accent-text)" : "var(--text-muted)", fontSize: "var(--text-2xs)", fontWeight: "var(--weight-semibold)", cursor: "pointer" }}>
                      ...
                      {{ wall: "קיר", mosaic: "פסיפס", feed: "פיד" }[l]}
                    </button>
                  );
                })}
              </div>
            ...
            {/* Content moderation */}
            <SectionCard title="ניהול תוכן" grow>
              ...
              {tab === "published" && (
                <div style={{ fontSize: "var(--text-2xs)", color: "var(--text-subtle)" }}>
                  טיפ: אפשר גם להצביע, להצמיד, להסתיר או להסיר תוכן ישירות על הלוח (בריחוף מעל כרטיס).
                </div>
              )}
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {listItems.length === 0 ? (
                  <div style={{ padding: "28px 10px", textAlign: "center", color: "var(--text-subtle)", fontSize: "var(--text-xs)" }}>
                    {tab === "pending" ? "אין תוכן שממתין לאישור" : tab === "hidden" ? "אין תוכן מוסתר" : tab === "rejected" ? "אין תוכן שנדחה" : "עוד אין תוכן — הוא יופיע כאן וברגע אמת על הלוח"}
                  </div>
                ) : (
                  listItems.map((s) => (
      ...
      <ConfirmDialog
        ...
        description="המשתתפים לא יוכלו לשלוח תוכן נוסף. כל התוכן שנאסף יישמר בהיסטוריית המפגשים."
